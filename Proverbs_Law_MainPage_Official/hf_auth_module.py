...
def create_login_interface(app_token_state: gr.State):
    ...
    Call this inside a gr.Tab() context - it renders inline, no wrapper needed.
...
- Expires: {session['expires'].strftime('%H:%M:%S')}
            ...
            return (
                ...
                gr.update(visible=False),
                gr.update(visible=False),
                gr.update(value=username, visible=True),
...
if __name__ == "__main__":
    # Test the login interface standalone
    with gr.Blocks() as demo:
        token_state = gr.State(None)
        create_login_interface(token_state)
